...
namespace detail
{
    ...
    void afterFork()
    {
        libmevent::CurrentThread::t_cachedTid = 0;
        libmevent::CurrentThread::t_threadName = "main";
        CurrentThread::tid();
    ...
    struct ThreadData
    {
        typedef libmevent::Thread::ThreadFunc ThreadFunc;
        ThreadFunc func_;
        string name_;
        pid_t* tid_;
        ...
        void runInThread()
        {
            *tid_ = libmevent::CurrentThread::tid();
            tid_ = NULL;
            latch_->countDown();
            latch_ = NULL;

            libmevent::CurrentThread::t_threadName = name_.empty() ? "libmeventThread" : name_.c_str();
            ::prctl(PR_SET_NAME, libmevent::CurrentThread::t_threadName);
            try {
                func_();
                libmevent::CurrentThread::t_threadName = "finished";
            } catch(const Exception& ex) {
                libmevent::CurrentThread::t_threadName = "crashed";
